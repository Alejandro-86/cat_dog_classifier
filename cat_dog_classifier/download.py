...
def main():
    ...
    folder = service.files().list(
        q=f"name contains '{sys.argv[1]}' and mimeType='application/vnd.google-apps.folder'",  # noqa E501
...
def get_full_path(service, folder):
    ...
    files = service.files().get(
        fileId=folder['parents'][0], fields='id, name, parents',
    ).execute()
    path = files['name'] + ' > ' + folder['name']
    while 'parents' in files:
        files = service.files().get(
            fileId=files['parents'][0], fields='id, name, parents',
        ).execute()
